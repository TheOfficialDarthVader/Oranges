...
cl_float fluid_viscosity = 0.1040;
...
int main() {
    ...
    float tau_d = (density*particle_diameter*particle_diameter)/(18*fluid_viscosity);
    sim_length = 100.0*tau_d;
    ...
    int sim_ret = runSim(hparticles, NUMPART, iterate_particle, particle_diameter, periodic, domain_length,
                         prefix, log_dir,
                         sim_length, timestep, VERBOSE, LOG_DATA, TRUE, log_step, device, context);
